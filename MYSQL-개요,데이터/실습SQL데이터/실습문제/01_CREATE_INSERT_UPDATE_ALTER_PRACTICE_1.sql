...
-- ORDER 는 SQL 에서 예약어 (order by), 그래서 백틱('') 로 감싸서 사용
CREATE TABLE `ORDER` (
...
-- cafe_order 테이블에서 status 가 preparing 인 테이블 번호와 주문 중인 금액 조회
select table_id, total_amount
from cafe_order
where status = 'preparing';

-- khtuser 데이터베이스에서 login 활동을 한 사용자들의 이름과 세부활동 정보 확인 / join on 사용
use khtuser;
...
-- update 문을 sql에서 직접적으로 실행할 때 문제가 되는 것을 방지하기 위해 safe 모드 생성
-- 테이블을 삭제하 때도 마찬가지로 safe 모드이므로 safe mode 해지 후 실행 가능
...
-- 테이블 자체 수정 tables
-- STATUS 안에는 'available' 'occupied'만 가능 으로 되어있음
-- enum('available', 'occupied')
-- create table set
-- create table enum

-- 테이블 자체 컬럼에 존재하는 규칙을 수정
-- alter table 수정할 테이블 명칭 modify column 컬럼명 컬렴명규칙
alter table tables
...
ENUM('preparing', 'completed', 'pending', ' cancelled', '주문 준비 중', '주문 완료' , '주문 대기 중', '주문 취소');
